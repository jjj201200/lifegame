function LifeGame(canvasId, lines, columns, livingCells) {
	...
	this.map = new Map(this.mapLines, this.mapColumns, livingCells);
	...
	this.createMap = function() {
		game.map = new Map(game.mapLines, game.mapColumns, game.livingCells);
		game.render();
	}
	this.clearMap = function() {
		game.map = new Map(game.mapLines, game.mapColumns, 0);
		game.render();
	}
	...
	this.ajustCellSize = function() {
		if (game.canvas.height / game.mapLines > game.canvas.width / game.mapColumns)
			game.cellWidth = game.cellHeight = game.canvas.width / game.mapColumns;
		else {
			game.cellWidth = game.cellHeight = game.canvas.height / game.mapLines;
		}
	}
	...
	this.execute = function() {
		for ( i = 0; i < game.mapLines; ++i) {
			for ( ii = 0; ii < game.mapColumns; ++ii) {
				...
				if (game.map.mapArray[i][ii].currentStatus == 0 && LivingCellsRound == 3) {
					...
					// console.log(game.map.mapArray[i][ii].currentStatus + ":+1")
	...
	this.getLivingCellsRound = function(i, ii) {
		...
		if (i == 0) {
			position[0][0] = position[1][0] = position[2][0] = game.mapLines - 1;
		}
		if (i == game.mapLines - 1) {
			position[5][0] = position[6][0] = position[7][0] = 0;
		}
		if (ii == 0) {
			position[0][1] = position[3][1] = position[5][1] = game.mapColumns - 1;
		}
		if (ii == game.mapColumns - 1) {
			position[2][1] = position[4][1] = position[7][1] = 0;
		}
		for ( iii = 0; iii < 8; ++iii) {
			// a = game.map.mapArray[position[iii][0]][position[iii][1]];
			// console.log(a);
			livingCellsRoundNum += game.map.mapArray[position[iii][0]][position[iii][1]].currentStatus;
		}
		// console.log(livingCellsRoundNum);
		// console.log("\n");
	...
	this.draw = function() {
		...
		$("#living_cells").html(game.map.livingCells);
		if (!game.map.livingCells) {
			game.stop();
			$("#startTaggle").html("start");
		}
		game.execute();
		...
		// console.log(game.map.livingCells+"\n");
