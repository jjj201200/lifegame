var Map = function(canvasId, lines, columns, livingCells) {
	...
	//当前监视细胞坐标
	this.watchCellX =null;
	this.watchCellY =null;
	...
	this.initialise = function() {
		for (var i = 0; i < this.lines; ++i) {
			...
			for (var ii = 0; ii < this.columns; ++ii) {
				this.mapArray[ i ][ii] = new Cell([i,ii],0,0);
				//this.mapArray[ i ][ii] = {
		...
		while (i >= 0) {
			...
			if (!this.mapArray[ tempn1 ][tempn2].currentStatus) {
				...
				this.mapArray[tempn1][tempn2].createNew();
			} else
				++i;
			--i;
		}
	...
	this.allExecute = function() {
		...
		for ( p = 0; p < this.lines; ++p) {
			for ( q = 0; q < this.columns; ++q) {
				...
				if (cell.currentStatus == 1) {
					if ((livingCellsAround < 2 || livingCellsAround > 3) && cell.hp > 0) {
						cell.nhp = cell.hp-this.subHP;
						if (cell.nhp <= 0) {
							cell.nhp = 0;
							cell.setNextLiving();
						}else
						...
						if (livingCellsAround == 3 && livingCellsLifeAround > 3*this.newCellHP*0.8) {
							cell.nhp=cell.hp + this.addHP;
						} else
							cell.nhp =cell.hp - this.subHP;
						this.dynamicArray.push(cell);
					}
				} else if (cell.currentStatus == 0 && livingCellsAround == 3 && livingCellsLifeAround > 3*this.newCellHP*0.9) {//复活
					cell.nhp = this.newCellHP;
					this.dynamicArray.push(cell);
				}
			}
		}
		...
		for ( i = 0; i < this.dynamicArray.length; ++i) {
			...
			//this.context.fillStyle = "rgba(" + Math.abs(255 - this.dynamicArray[i]["hp"]) + "," + Math.abs(this.dynamicArray[i]["hp"]) + ",0," + Math.abs(this.dynamicArray[i]["hp"] / 127) + ")";
			this.context.fillStyle = "rgba(255," +( Math.abs(this.dynamicArray[i].hp)) + ",255,1)";
			this.dynamicArray[i].currentStatus = 1;
			this.context.fillRect(this.dynamicArray[i].x * this.cellWidth + this.translateX, this.dynamicArray[i].y * this.cellHeight + this.translateY, this.cellWidth, this.cellHeight);
		}
		// if (this.mapSize < 100000) {
		this.drawBorder();
		// }
		for ( i = 0; i < this.dieArray.length; ++i) {
			this.dieArray[i].setCurrentDie();
		}
	};
	this.execute = function() {
		...
		while (i >= 0) {//通过调整队列生成下一步的存活队列
			...
			if (this.adjustArray[i].currentStatus) {//当前为活细胞
				if ((this.adjustArray[i]["livingCellsAround"] < 2 || this.adjustArray[i]["livingCellsAround"] > 3) && this.adjustArray[i]["hp"] > 0) {//周围没有充足活细胞且hp未为0
					this.adjustArray[i]["hp"] -= this.subHP;
					this.adjustArray[i].currentStatus = 1;

					if (this.adjustArray[i]["hp"] <= 0) {//活细胞hp为0
						this.adjustArray[i]["hp"] = 0;
						this.adjustArray[i].currentStatus = 0;
					}else
						this.dynamicArray.push(this.adjustArray[i]);
				}  else {
					if (this.adjustArray[i]["livingCellsAround"] == 3 && this.adjustArray[i]["livingCellsLifeAround"] > 3*this.newCellHP*0.8) {
						this.adjustArray[i]["hp"] += this.addHP;
					} else
						this.adjustArray[i]["hp"] -= this.subHP;
					this.adjustArray[i].currentStatus = 1;
					this.dynamicArray.push(this.adjustArray[i]);
				}
			} else if (!this.adjustArray[i].currentStatus && this.adjustArray[i]["livingCellsAround"] == 3 && this.adjustArray[i]["livingCellsLifeAround"] > 3*this.newCellHP*0.9) {
				this.adjustArray[i]["hp"] = this.newCellHP;
				this.adjustArray[i].currentStatus = 1;
				this.dynamicArray.push(this.adjustArray[i]);
			}
	...
	this.draw = function() {
		...
		if(this.watchCellX!=null){
			$('hp').value=lifegame.map.mapArray[this.watchCellY][this.watchCellX].hp;
			$('cs').value=lifegame.map.mapArray[this.watchCellY][this.watchCellX].currentStatus;
		}
		if (0) {
